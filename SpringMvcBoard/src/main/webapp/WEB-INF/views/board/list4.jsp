...
<a href="<c:url value='/board/content4/${b.boardNo}${param.page == null ? pc.makeURI(1) : pc.makeURI(param.page)}' />">
...
<div align="center">
		...
		<c:if test="${pc.prev}"> <!--  이전버튼이 true일때만 등장. -->
					<a href="<c:url value='/board/list4${pc.makeURI(pc.beginPage - 1)}'/>" 
						>이전</a>
		...
		<c:forEach var="pageNum" begin="${pc.beginPage}" end="${pc.endPage}">
						...
						<li class="grid_item"><a
							href="<c:url value='/board/list4${pc.makeURI(pageNum)}'/>"
							class="${(pc.paging.page == pageNum) ? 'page-active' : ''}" 
							>${pageNum}</a>
						</li>
					</c:forEach>
					<c:if test="${pc.next}"> <!-- 이것도, next가 true일때만 등장. -->
					<a href="<c:url value='/board/list4${pc.makeURI(pc.endPage + 1)}'/>"
						>다음</a>
				
					</c:if>	
				<a href="<c:url value='/board/write4?page=${p.page}&boardNo=${b.boardNo}'/>">등록</a> <!--  p태그를 하면 줄개행이 된다. -->
