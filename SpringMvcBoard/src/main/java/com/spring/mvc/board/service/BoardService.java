...
public class BoardService implements IBoardService {
	...
	public int addNewArticle3(Map articleMap) throws Exception{
		...
		int boardNo2 = (Integer)articleMap.get("articleNO");
		
		
		
		int articleNO = selectNewArticleNO3();
		//int articleNO2 = (Integer)articleMap.get("articleNO");
		//int articleNO = selectNewArticleNO3();
		articleMap.put("boardNo", articleNO);
		
		System.out.println("서비스에서 boardNo" + boardNo);
		
		int imageFileNO = selectNewImageFileNO3();
		for(ImageVO imageVO : imageFileList){
			imageVO.setImageFileNO(++imageFileNO);
			imageVO.setBoardNo(boardNo2);
		}
